...
// TODO: read the access token from AuthProvider once it lands in Phase 7.
// Reading directly from localStorage is a stopgap until then.
...
export default function AdminAboutSettingsPage() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        ...
        const admin = await apiRequest<AuthenticatedAdminResponse>("/v1/auth/me", {
          headers: authHeaders(token),
        });
        if (cancelled) return;
  ...
  async function onSave(data: ProfileFormValues) {
    ...
    try {
      const token = getAccessToken();
      await apiRequest("/v1/auth/profile", {
        method: "PATCH",
        headers: authHeaders(token),
        body: JSON.stringify({
